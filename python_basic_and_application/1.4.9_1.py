...
def create(lst, ns, parent):
    if ns not in lst:
        lst.setdefault(ns, {'parent': parent, 'vars': []})
    return lst


def add(lst, ns, var):
    lst[ns]['vars'].append(var)
    return lst


def get(lst, ns, var):
    if ns != None:
        if var in lst[ns]['vars']:
            return ns
        else:
            return get(lst, lst[ns]['parent'], var)
    else:
        return None
...
for i in range(int(input())):
    ...
    if cmd == 'add':
        add(nss, ns, var)
    if cmd == 'get':
        print(get(nss, ns, var))

# print(nss)

# Это было очень сложно для меня, но Я справился!
